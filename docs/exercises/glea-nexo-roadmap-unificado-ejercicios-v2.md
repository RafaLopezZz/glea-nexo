...
El Bloque 2 queda formalmente cerrado. La semántica temporal del sistema quedó aclarada y documentada, el contrato temporal fue endurecido y verificado con pruebas, y la idempotencia de ingest fue validada con evidencia real en API y SQL.

La conclusión de cierre es que la deduplicación actual debe clasificarse como `PARCIAL`: evita duplicados funcionales cuando se reutiliza la misma clave técnica, pero no detecta conflicto semántico si un productor reutiliza un `messageId` con payload distinto. El riesgo residual principal queda identificado y aceptado para esta fase, con seguimiento pendiente en preparación de demo técnica y futuras tareas de hardening.
...
### TO-DOs pendientes / Ideas de mejora no incluidas en ejercicios
